...
var valorBloqueHole = 100;
var valor360Ecuacion = 360;
...
function base() {
    blocks = setInterval(function(){
        ...
        if(blockLastTop<valorMaximoContenido||counter==0){
            ...
            block.style.top = blockLastTop + valorBloqueHole + "px";
            hole.style.top = holeLastTop + valorBloqueHole + "px";
            var random = Math.floor(Math.random() * valor360Ecuacion);
        ...
        if(characterTop <= 0){
            ...
            while (contador < currentBlocks.length){
                ...
                iblock.style.top = iblockTop -  medio + "px";
                ihole.style.top = iblockTop -  medio + "px";
                if(iblockTop < - sumaIholeLeft){
                    currentBlocks.shift();
                    iblock.remove();
                    ihole.remove();
                }
                if(iblockTop- sumaIholeLeft<characterTop && iblockTop>characterTop){
                    drop++;
                    if(iholeLeft<=characterLeft && iholeLeft+ sumaIholeLeft>=characterLeft){
                        drop = 0;
                    }
                }
                contador++;
            ...
            if(drop==0){
                if(characterTop < valorEscalera){
                    character.style.top = characterTop + valor2 + "px";
                    if (!endGame) {
                        var playerRef = firebase.database().ref(`players/${initPlayer}`);
                        playerRef.update({
                            y: characterTop + valor2
                        });
                    }
                }
            }else{
                character.style.top = characterTop -  medio + "px";
                if (!endGame) {
                    var playerRef = firebase.database().ref(`players/${initPlayer}`);
                    playerRef.update({
                        y: characterTop -  medio
                    });
                }
            }
